...
/**
 * Nguồn dữ liệu chính: Supabase, bảng `articles` (xem `supabase/schema.sql`).
 * File này sinh lại bằng `npm run content:sync` sau khi nội dung được biên
 * tập trong Supabase Studio. Hiện vẫn là dữ liệu seed gốc viết tay vì chưa
 * có project Supabase nào kết nối — sửa trực tiếp ở đây vẫn an toàn cho tới
