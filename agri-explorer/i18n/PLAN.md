...
## Mục 7 — Backend .NET 8 (Flat Architecture, MSSQL) — thay Supabase

Dự án `AgriExplorerApi` (.NET 8, Minimal hosting + Controllers, flat folder
Entities/Data/Dtos/Controllers/Auth) được tạo **bên ngoài** repo Next.js này,
thay hoàn toàn Supabase. DB: MSSQL Server (EF Core, theo yêu cầu thay vì
Postgres ban đầu đề xuất).

**Đã xong:**
- [x] Tạo project + cấu trúc flat, package EF Core SqlServer/JWT/BCrypt
- [x] Entities (Plant, Animal, MediaItem, Article, SeasonalEvent, WeatherAlert, AppUser) + `AppDbContext` (enum→string, FK, check constraint, giữ đúng giá trị kebab-case category cũ)
- [x] `Program.cs`: JWT Bearer auth, CORS cho FE, Swagger, auto-migrate + seed admin user lúc start
- [x] Controllers GET (public) + POST upsert (role `admin`) cho 6 resource, DTO request/response khớp đúng camelCase domain type của FE (không lộ entity raw)
- [x] FE: `lib/db/*` đổi từ Supabase SDK → `fetch` sang AgriExplorerApi (`lib/db/client.ts` mới: `apiGet`/`apiPost` + JWT login), validate lại bằng Zod schema sẵn có — giữ nguyên kiến trúc build-time sync (Mục 6b), **không phải sửa** 19 file component đang import `lib/data/*`
- [x] `.env.example`, gỡ `@supabase/supabase-js` khỏi `package.json`
- [x] Verify FE: `tsc --noEmit` sạch, `content:seed`/`content:sync --dry-run` chạy đúng logic

**Chưa làm / cần làm tiếp:**
- [ ] `dotnet restore && dotnet build` thật (sandbox không có mạng tới nuget.org, **chưa build được** — xem `AgriExplorerApi/README.md`)
- [ ] `dotnet ef migrations add Init` + chạy thật trên MSSQL, test 6 endpoint qua Swagger
- [ ] Chạy `npm run content:seed` thật (cần `AGRI_API_URL`/`AGRI_API_USERNAME`/`AGRI_API_PASSWORD` trong `.env.local`) để đẩy data từ `lib/data/*.ts` lên MSSQL, sau đó `npm run content:sync` để xác nhận round-trip đúng
- [ ] Xoá `supabase/` folder sau khi đã xác nhận seed thành công (archive `schema.sql` làm tài liệu DDL gốc nếu cần)
