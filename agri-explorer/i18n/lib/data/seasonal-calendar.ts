...
/**
 * Nguồn dữ liệu chính: AgriExplorerApi (.NET, MSSQL), bảng `seasonal_events` + `weather_alerts`
 * (xem `supabase/schema.sql`). File này sinh lại bằng `npm run content:sync`
 * sau khi nội dung được biên tập qua AgriExplorerApi. Hiện vẫn là dữ liệu
 * seed gốc viết tay vì chưa có AgriExplorerApi nào kết nối.
 *
