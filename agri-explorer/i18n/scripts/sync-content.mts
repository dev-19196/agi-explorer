 ...
 *
 * Đọc nội dung mới nhất từ AgriExplorerApi (.NET) và SINH LẠI các file `lib/data/*.ts`
 ...
 *
 * Chạy script này sau khi biên tập nội dung trong AgriExplorerApi (qua admin script/Swagger), trước
 * khi `npm run build` để build production với nội dung mới nhất.
 *
 * Cách dùng:
 *   npm run content:sync              — đọc AgriExplorerApi (.NET), ghi đè lib/data/*.ts
...
/**
 * Nguồn dữ liệu chính: AgriExplorerApi (.NET), resource ${opts.table} (xem
 * \`supabase/schema.sql\`). File này được SINH TỰ ĐỘNG bởi
 * \`npm run content:sync\` lúc ${generatedAt} — KHÔNG sửa tay, mọi thay đổi
 * sẽ bị ghi đè ở lần sync kế tiếp. Sửa nội dung trong AgriExplorerApi (qua admin script/Swagger) rồi
...
async function main() {
  console.log(`\n=== Đồng bộ nội dung từ AgriExplorerApi (.NET) ${isDryRun ? "(DRY RUN — không ghi file)" : ""} ===\n`);
