 ...
 * lên AgriExplorerApi (.NET) — chạy 1 lần (hoặc bất cứ khi nào muốn reset nội dung gốc)
 * sau khi AgriExplorerApi (.NET) đã chạy migration (xem AgriExplorerApi/README.md).
 *
 * Idempotent: dùng `upsert` theo khoá chính (slug/id), chạy lại nhiều lần
 * không tạo bản trùng.
 *
 * Cách dùng:
 *   npm run content:seed              — seed thật lên AgriExplorerApi (.NET)
...
async function main() {
  console.log(`\n=== Seed dữ liệu lên AgriExplorerApi (.NET) ${isDryRun ? "(DRY RUN — không ghi gì)" : ""} ===\n`);
